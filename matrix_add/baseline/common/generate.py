...
a = rng.normal(0, 10, (10, 512)).astype(np.float32)
b = rng.normal(0, 10, (10, 512)).astype(np.float32)
a[0] = 0; b[0] = 0
b[1] = -a[1]
b[2] = 0
# Distinct row/column coordinates expose row indexing and missing tail rows.
a[3] = np.arange(512, dtype=np.float32); b[3] = -256
a[4::3] = np.arange(4, 10, 3, dtype=np.float32)[:, None]
a.tofile(args.a); b.tofile(args.b)
print(f"Generated A and B: shape=(10,512), FP32, seed={args.seed}")
