# 20x1024 softmax: row loop with unrolled vector operations
...
The kernel runs one vector block, processes 1 batch(es) of 20
rows, and returns all 20480 normalized FP32 values. UB input/output offsets,
DMA sizes and barriers match the sibling variants: input at 0, output at
163840, highest used address ending at 245760 bytes. The old
...
Expected data loads/stores are 320/320, matching fully unrolled and
avoiding baseline's 960/640. Row offsets are calculated at runtime;
...
```bash
cd ~/npu-tutorial/softmax/unroll_20x1024/row_loop
bash run.sh
