# 16x1024 softmax: row loop with unrolled vector operations
...
The kernel runs one vector block, processes 1 batch(es) of 16
rows, and returns all 16384 normalized FP32 values. UB input/output offsets,
...
Expected data loads/stores are 256/256, matching fully unrolled and
avoiding baseline's 768/512. Row offsets are calculated at runtime;
...
```bash
cd ~/npu-tutorial/softmax/unroll_16x1024/row_loop
bash run.sh
