# Row softmax: 16x1024 loops versus register reuse

Both CCE kernels return full normalized 16x1024 FP32 softmax on one A5
...
- `unrolled`: explicit operations for all 16 rows, with no source loops;
...
Both process 1 batch(es) of 16 rows. Each batch uses one
...
is 65536 bytes each. The 16-row output is never transposed.
...
Expected loads/stores: baseline 768/512, unrolled 256/256; both
perform 256 vector exponentials/divisions and 16 horizontal maxima/sums.
This tests unrolling and explicit register reuse together.

## Run and check

```bash
cd ~/npu-tutorial/softmax/unroll_16x1024/baseline
bash run.sh
cd ../unrolled
bash run.sh
```
