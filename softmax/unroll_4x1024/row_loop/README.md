# 4x1024 softmax: row loop with unrolled vector operations
...
The kernel runs one vector block, processes 1 batch(es) of 4
rows, and returns all 4096 normalized FP32 values. UB input/output offsets,
DMA sizes and barriers match the sibling variants: input at 0, output at
32768, highest used address ending at 49152 bytes. The old
...
Expected data loads/stores are 64/64, matching fully unrolled and
avoiding baseline's 192/128. Row offsets are calculated at runtime;
...
```bash
cd ~/npu-tutorial/softmax/unroll_4x1024/row_loop
bash run.sh
