"""Validate all 4x1024 results, optionally comparing the other backend too."""
import argparse
from pathlib import Path
import numpy as np

def load(path):
    if Path(path).stat().st_size != 4 * 1024 * 4:
        raise ValueError(f"{path}: expected exactly 16384 bytes (4x1024 FP32)")
    return np.fromfile(path, dtype=np.float32).reshape(4, 1024)
