...
int main(int argc, char** argv) {
    ...
    constexpr size_t count = 8 * 1024, bytes = count * sizeof(float);
    ...
    try {
        ...
        if (!file || file.tellg() != std::streampos(bytes)) throw std::runtime_error("Input must be 8x1024 FP32");
        ...
        std::printf("Saved 8x1024 FP32 output to %s\n", argv[2]);
        status = 0;
