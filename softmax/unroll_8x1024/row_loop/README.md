# 8x1024 softmax: row loop with unrolled vector operations
...
The kernel runs one vector block, processes 1 batch(es) of 8
rows, and returns all 8192 normalized FP32 values. UB input/output offsets,
DMA sizes and barriers match the sibling variants: input at 0, output at
65536, highest used address ending at 98304 bytes. The old
...
Expected data loads/stores are 128/128, matching fully unrolled and
avoiding baseline's 384/256. Row offsets are calculated at runtime;
...
```bash
cd ~/npu-tutorial/softmax/unroll_8x1024/row_loop
bash run.sh
