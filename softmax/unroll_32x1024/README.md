# Row softmax: 32x1024 loops versus register reuse

Both CCE kernels return full normalized 32x1024 FP32 softmax on one A5
...
- `unrolled`: explicit operations for all 32 rows, with no source loops;
...
Both process 2 batch(es) of 16 rows. Each batch uses one
65536-byte GM input DMA and one equally sized output DMA, with matching
barriers and offsets in both versions. Total logical input and output traffic
is 131072 bytes each. The 32-row output is never transposed.

The shared UB layout has input at 0, baseline exponentials at 65536,
and output at 131072, ending at 196608 bytes. The unrolled kernel
...
Expected loads/stores: baseline 1536/1024, unrolled 512/512; both
perform 512 vector exponentials/divisions and 32 horizontal maxima/sums.
This tests unrolling and explicit register reuse together.

## Run and check

```bash
cd ~/npu-tutorial/softmax/unroll_32x1024/baseline
bash run.sh
cd ../unrolled
bash run.sh
```
