# Validation: retained row loop, 32x1024 softmax
...
| --- | ---: | ---: | ---: |
| Total ticks | 16291 | 18587 | 15750 |
| Summed VF duration (us) | 2.595 | 3.821 | 2.254 |

The new run passed all 32768 outputs against stable NumPy float64 softmax.
...
Measured data vector loads/stores: **512/512**, matching the
fully unrolled access counts without extra UB spill traffic. The profiler
reports **2 vector-loop setup instruction(s)**: one per batch,
confirming the row loop was retained. There are 512 vector EXP,
512 vector DIV, and 32 horizontal max/sum instructions each.
...
Summed input/output DMA durations: 2.940/2.738 us.
Model wall time: 98.7578 seconds; concurrent runs include host contention.
...
Artifact directory relative to this variant: `build/sim-kyfIni1V/`.
