"""Validate all 32x1024 results, optionally comparing the other backend too."""
import argparse
from pathlib import Path
import numpy as np

def load(path):
    if Path(path).stat().st_size != 32 * 1024 * 4:
        raise ValueError(f"{path}: expected exactly 131072 bytes (32x1024 FP32)")
    return np.fromfile(path, dtype=np.float32).reshape(32, 1024)
