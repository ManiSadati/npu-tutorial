...
[[ -f "$PTO_ISA_ROOT/include/pto/pto-inst.hpp" ]] || { echo "Set PTO_ISA_ROOT to the PTO-ISA checkout." >&2; exit 1; }
mkdir -p "$BUILD_DIR"
exec > >(tee "$BUILD_DIR/build.log") 2>&1
"$CCEC" -fPIC -shared --cce-fatobj-link --cce-aicore-arch="$DEVICE_ARCH" \
  -DREGISTER_BASE -O2 -std=c++17 -I "$PTO_ISA_ROOT/include" \
...
"${CXX:-g++}" -O2 -std=c++17 "$LAB_ROOT/../common/host.cpp" \
...
printf 'Build passed: %s\n' "$BUILD_DIR/verify"
{
  ...
  sha256sum "$LAB_ROOT"/src/*.cpp "$LAB_ROOT/../common/host.cpp" "$LAB_ROOT"/{build,env,run}.sh
