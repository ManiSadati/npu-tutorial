# Row softmax: 2x1024 loops versus register reuse

Both CCE kernels return full normalized 2x1024 FP32 softmax, on one A5
vector block. They use one 8192-byte input DMA and one equally sized
...
- `unrolled`: explicit operations for all 2 rows, no source loops; each row's
...
Both retain the same UB output offset 16384 and address span 24576 bytes.
...
Expected vector loads/stores: baseline 96/64, unrolled 32/32.
Both perform 32 exponentials, 32 divisions, and 2 horizontal maxima
and sums. The comparison changes register reuse and unrolling together.

```bash
cd ~/npu-tutorial/softmax/unroll_2x1024/baseline
bash run.sh
cd ../unrolled
bash run.sh
```
