# 2x1024 softmax: row loop with unrolled vector operations
...
The kernel runs one vector block, processes 1 batch(es) of 2
rows, and returns all 2048 normalized FP32 values. UB input/output offsets,
DMA sizes and barriers match the sibling variants: input at 0, output at
16384, highest used address ending at 24576 bytes. The old
...
Expected data loads/stores are 32/32, matching fully unrolled and
avoiding baseline's 96/64. Row offsets are calculated at runtime;
...
```bash
cd ~/npu-tutorial/softmax/unroll_2x1024/row_loop
bash run.sh
